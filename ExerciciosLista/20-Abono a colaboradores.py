...
stop = 0
print("Salário    - Abono")
while stop < len(salarios):
    print(f"R${salarios[stop]:>8} - R$ {abonos[stop]:>6,.2f}")
    if abonos[stop] == 100:
        funcionariosValorM += 1
    stop += 1

print(f"\nForam processados {totalFuncionarios} colaboradores \nTotal gasto com abonos: R$ {valorTotal:,.2f} \nValor mínimo pago a {funcionariosValorM} colaboradores \nMaior valor de abono pago: R$ {max(abonos):,.2f}")